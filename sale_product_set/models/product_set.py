...
class ProductSet(models.Model):
    ...
    ref = fields.Char(string="Internal Reference",
                      help="Product set internal reference", copy=False)
    ...
    def name_get(self):
        '''Give name list in name,Reference,Partner'''
        return [(rec.id, rec._name_get()) for rec in self]

    def _name_get(self):
        '''Prepare list for name,Reference,Partner'''
        parts = []
        self.ref and parts.append("[%s]" % self.ref)
        parts.append(self.name)
        self.partner_id and parts.append("@ %s" % self.partner_id.name)
        return " ".join(parts)
