...
class TestProductSet(common.SavepointCase):
    ...
    def test_add_set_lines_init(self):
        ...
        self.assertEqual(wiz.product_set_line_ids,
                         self.product_set.set_line_ids)
        self.assertEqual(
            [x.id for x in wiz._get_lines()], self.product_set.set_line_ids.ids
        )
        # Pass via ctx
        line_ids = self.env.ref(
            "sale_product_set.product_set_line_computer_1").ids
        ...
        # Pass at create
        line_ids = self.env.ref(
            "sale_product_set.product_set_line_computer_3").ids
        wiz = self._get_wiz()
    ...
    def test_add_set(self):
        ...
        seq_line1 = sequence.pop(
            self.env.ref(
                "sale_product_set.product_set_line_computer_4").product_id.id
        )
        seq_line2 = sequence.pop(
            self.env.ref(
                "sale_product_set.product_set_line_computer_1").product_id.id
        )
        seq_line3 = sequence.pop(
            self.env.ref(
                "sale_product_set.product_set_line_computer_3").product_id.id
        )
        self.assertTrue(
            max([v for k, v in sequence.items()]
    ...
    def test_add_set_on_empty_so(self):
        so = self.so_model.create({"partner_id": self.ref(
            "base.res_partner_1")})
    ...
    def test_name(self):
        ...
        product_set.ref = ""
        self.assertEqual(product_set.name_get(), [
            (product_set.id, "Foo")])
        # with ref
        product_set.ref = "123"
        self.assertEqual(product_set.name_get(), [
