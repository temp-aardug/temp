...
class ProductSetAdd(models.TransientModel):
    ...
    product_set_line_ids = fields.Many2many(
        "product.set.line", "Product set lines",
        required=True, ondelete="cascade",
        compute="_compute_product_set_line_ids", readonly=False)
    ...
    # Context is not passed from anywhere, need to check for that
    @api.depends_context("product_set_add__set_line_ids")
    @api.depends("product_set_id")
    def _compute_product_set_line_ids(self):
        '''set product set lines base on wizard product set selection'''
        ...
        for rec in self:
            ...
            if lines:
                # Use the ones from ctx but make sure they belong to
                # the same set.
    ...
    def _get_max_sequence(self):
        '''Get maximum order line sequence'''
        return self.order_id.order_line and max(
            [line.sequence for line in self.order_id.order_line]) or 0

    def _get_lines(self):
        '''Get Existing line from current so,to skip if select
        skip_existing or return'''
        ...
        for set_line in self.product_set_line_ids:
            if self.skip_existing_products and \
                    set_line.product_id.id in so_product_ids:
                continue
            yield set_line

    def prepare_sale_order_line_data(self, set_line, max_sequence=0):
        '''Prepare order line data with configured data from set line.like
        self quantity multiply with line quantity and sequence + max_sequence
        return lines data'''
        self.ensure_one()
        line_values = set_line.prepare_sale_order_line_values(
            self.order_id, self.quantity, max_sequence=max_sequence)
        line_values.update(self.env["sale.order.line"].play_onchanges(
            line_values, line_values.keys()))
        return line_values

    def _prepare_order_lines(self):
        '''Prepare order lines with max sequence and existing products.'''
        order_lines = []
        for set_line in self._get_lines():
            order_lines.append((0, 0, self.prepare_sale_order_line_data(
                set_line, max_sequence=self._get_max_sequence())))
        return order_lines
    ...
    def add_set(self):
        ...
        order_lines and self.order_id.write({"order_line": order_lines})
